...
module.exports = (interaction, date, sportType, shouldReply = true) => {
    if (date.length !== 8) {
    ...
    if (interaction.commandName === 'picks') {
        ...
        parseDateAndSend(interaction, date, `${getCommandSportIcon(sportType)} **AI ${sportType.toUpperCase()} GAME PICKS SCHEDULED FOR: ** *DATE*`, shouldReply)
        return { sportType, gamesUrl, standingsUrl, teamStatsUrl }
    } else if (interaction.commandName === 'summary') {
        const gamesUrl = (!sportType.includes('ncaab')) ? process.env.API_GENERAL_URL.replace('SPORTTYPE', sportType).replace('DATE', date)
            : process.env.API_NCAAB_URL.replace('DATE', date),
            standingsUrl = process.env.API_GENERAL_STANDINGS_URL.replace('SPORTTYPE', sportType)
        const ymd = parseDateAndSend(interaction, date, `${getCommandSportIcon(sportType)} **${sportType.toUpperCase()} SUMMARY OF: ** *DATE*`, shouldReply)
        ...
        parseDateAndSend(interaction, date, `${getCommandSportIcon(sportType)} **${sportType.toUpperCase()} SCHEDULED FOR: ** *DATE*`, shouldReply)
        return { sportType, gamesUrl }
    }
}

function parseDateAndSend(interaction, date, message, shouldReply) {
